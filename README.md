...
1. **Класиране** — таблица на група (или подиум + една таблица при плосък формат)
2. **Предстоящи** — планирани мачове по дата с relative pill-ове ("Утре", "След 3 дни")
3. **Двубои (H2H)** — разгъващи се карти с противниците от групата
4. **История** — приключилите турнири: финално класиране + всички резултати
...
### Формат на турнира — групи

Полето `tournament` описва текущия формат. Когато е `null`, турнирът е плосък round-robin върху целия roster (оригиналното поведение).
...
**Мачове = двойките вътре в групата ∪ `extraPairs`.** При 2 групи по 5 това е 2 × C(5,2) = **20 мача**, не C(10,2) = 45. Крос-групова двойка не е мач — сървърът връща `404` за нея.

`extraPairs` е плейоф стъпалото. Когато груповата фаза приключи и лидерите трябва да играят финал, добавяш `"Сашо|Никата"` в масива — **промяна в данните, не в кода**. Плейоф мачовете не влизат в груповите класирания (те имат `stage: 'playoff'`).

Класирането е **на група**, с ранг вътре в групата: при отделни групови round-robin-и общото подреждане би сравнявало резултати, които никога не са се срещали.

### История — `archive`

Приключилите турнири се пазят като замразени снапшоти:
...
"archive": [{
  ...
  "endedAt": "2026-07-31",
  "players": [ /* 21 имена — СВОЯТ roster на снапшота */ ],
  "results": { /* 195 */ },
  "resultsRecordedAt": { /* ... */ },
  "groups": null
}]
...
**Всеки снапшот носи собствен `players` масив.** Каноничният ред на ключа зависи от позицията в roster-а: `"Иво|Сашо"` е правилно за 21-играчния списък от пролетта, докато днешният 10-играчен би дал `"Сашо|Иво"`. Четенето на снапшота със **своя** roster прави старите ключове валидни завинаги.

Архивът е **read-only**. Преименуване или изтриване на играч днес пипа `players` и `tournament`, но никога `archive` — историята записва кой реално е играл.

Смяната на турнир е ръчна и изрична: Експорт → трансформация → Импорт. Няма бутон „приключи турнира" и няма имплицитна миграция при зареждане.
...
- В Админ → ⋯ меню → **задръж пръст върху „⬇ Експорт“ ~1.5 сек** → отваря се екранът за импорт → избери JSON файл → потвърди
  (Импортът няма собствен бутон нарочно — презаписва цялата база и един случаен тап би бил необратим.)
...
- Админ → ⋯ → задръж „⬇ Експорт“ ~1.5 сек → Импорт
